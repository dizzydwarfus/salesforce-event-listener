...
SANDBOX_TOKEN_URL = (
    "https://basf3dps--uat.sandbox.my.salesforce.com/services/oauth2/token"
)
...
class AuthenticatorBase(AuthExtension):
    ...
    def _token_url(self) -> str:
        ...
        if self._sandbox:
            return self._domain + "/services/oauth2/token"
        return self._domain + "/services/oauth2/token"
...
class PasswordAuthenticator(AuthenticatorBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            domain=domain, sandbox=sandbox, json_dumps=json_dumps, json_loads=json_loads
        )
        #: OAuth2 client id
...
class RefreshTokenAuthenticator(AuthenticatorBase):
    ...
    def __init__(
        ...
        """
        :param domain: Domain of the Salesforce org
        :param consumer_key: Consumer key from the Salesforce connected \
        app definition
        :param consumer_secret: Consumer secret from the Salesforce \
        connected app definition
